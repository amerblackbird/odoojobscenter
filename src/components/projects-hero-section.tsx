...
export function ProjectsHeroSection() {
  const projectIcons = [
    { icon: <CheckCircle className="h-6 w-6" />, color: "text-green-600", bgColor: "bg-green-50" },
    { icon: <TrendingUp className="h-6 w-6" />, color: "text-blue-600", bgColor: "bg-blue-50" },
    { icon: <Award className="h-6 w-6" />, color: "text-purple-600", bgColor: "bg-purple-50" },
    { icon: <Users className="h-6 w-6" />, color: "text-orange-600", bgColor: "bg-orange-50" },
  ]
    ...
    <section className="pt-24 pb-16 bg-gradient-to-br from-gray-50 to-white overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            Showcasing our successful implementations with measurable outcomes, client satisfaction, and transformative
          ...
          >
            {projectIcons.map((project, index) => (
              <motion.div
                key={index}
                className={`w-16 h-16 ${project.bgColor} rounded-2xl flex items-center justify-center ${project.color}`}
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
